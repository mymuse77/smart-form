...
> 状态说明：本文同时包含目标态与后续规划，并非所有条目都已实现。当前代码事实以根目录 `README.md` 和 `docs/开发运行文档和部署文档.md` 为准；敏感页面暂停/脱敏与服务端截图纯内存转发，本轮明确未实现。
>
> 文档版本：V1.3
> 状态：MVP 设计基线
> 更新日期：2026-07-25
> MVP 范围：网站数据采集
> 后续范围：智能表单填报
> 技术栈约束：核心业务 TypeScript；本地 Browser Use Python Sidecar；B/S 不使用 React；LLM 使用 OpenAI 兼容协议，默认 DeepSeek

## 1. 文档结论

当前需求已经具备进入技术验证和 MVP 实施的条件。架构方向已确定；阶段 0 技术 Spike 仍是进入全量开发的门禁。
...
5. Browser Use Explore Agent 结合 LLM 负责未知站点的首次探索、失败诊断和路径修复。
...
13. B/S、Companion Agent、Playwright Runner、服务端与共享契约使用 TypeScript monorepo；Browser Use 作为受控的 Python Sidecar，仅承担探索与 LLM 工具循环；B/S 不引入 React。
...
7. 独立回放脚本并验证结果。
8. 将验证通过的能力包上传服务端。
9. 后续同类任务优先复用能力包，失败时再回退到 Browser Use Explore Agent。
...
| 语言与工程 | TypeScript monorepo + Python Sidecar | Web、客户端、服务端、Runner 与契约包使用 TypeScript；Browser Use 仅在本地锁定 Python 环境运行 |
| Web 前端 | Vue 3 + TypeScript + Vite | 承载 Chat、只读工作区、执行过程和结果管理；不使用 React |
| 客户端技术 | Electron 最小壳 + Node.js/TypeScript | 托盘、设备绑定、授权、本地编排和自动升级 |
| 浏览器内核 | Playwright 管理的可见 Chromium | 独立 Profile，人工介入时直接切到原生窗口 |
| 智能探索 | Browser Use Python Sidecar | 与桌面主进程隔离，连接任务专属 Chromium；通过 OpenAI 兼容协议调用默认 DeepSeek 模型 |
| 稳定执行 | Playwright TypeScript | 与客户端和本地编排器同一技术栈；与 Browser Use 通过 Action Trace 解耦 |
...
| 本地进程协议 | Loopback WebSocket 或 Node IPC | 主进程与 Node Runner 可使用 IPC；Browser Use Sidecar 使用版本化 JSON Schema 消息 |
...
1. 验证 Browser Use Python Sidecar 通过 Playwright/CDP 连接任务专属可见 Chromium。
2. 验证 Agent 能稳定锁定任务专属 Browser Context 和 Page，不误操作其他页面。
3. 验证 Browser Use Explore Agent 与 Playwright Runner 在同一任务浏览器上的控制权切换和 API 覆盖范围。
...
8. 验证 TypeScript 共享契约（任务 Schema、事件、能力包）可在 Web、客户端与服务端直接复用，并可由 Sidecar 的 JSON Schema 客户端校验。

截图流的 MVP 基线方案为：

- Browser Use Explore Agent 在 Playwright 管理的可见 Chromium 窗口中完成探索。
...
| LLM 数据处理 | 首次使用前明确提示：脱敏页面摘要会发送至配置的 DeepSeek 服务；未同意则不启动探索 | 阶段 1 开始前 |
...
- Browser Use Explore Agent 可以动态判断、尝试和修正。
- Playwright 脚本必须确定、可测试、可审计。
- 不把 Agent 的自然语言推理直接当成可复用能力。

### 4.3 TypeScript 核心系统与 Browser Use Sidecar

- B/S、Companion Agent、Playwright Runner、服务端、验证 Worker 与共享包统一 TypeScript。
- Browser Use 以 Python Sidecar 运行，仅接收经 Schema 校验的任务快照与控制命令，并只通过版本化 JSON IPC 返回事件和语义轨迹。
- 契约、事件、任务 Schema、能力包类型在 TypeScript monorepo 中单源维护；Sidecar 使用生成的 JSON Schema，不共享内部 TypeScript 模块。
- B/S 使用 Vue 3 + TypeScript，不引入 React / React 生态状态库。
- Python 运行时只允许存在于本地 Browser Use Sidecar，不进入 Web、服务端、Playwright Runner 或能力包运行时。
- Sidecar 必须使用锁定的 Python 与 Browser Use 版本，并与主进程保持进程隔离。
...
flowchart LR
    ...
    subgraph LOCAL[用户本地执行面]
        ...
        ORCH --> BU[Browser Use Explore Agent<br/>Python Sidecar + LLM]
        ORCH --> PR[Playwright Runner]
        ORCH --> AC[Action Trace Compiler]
        ORCH --> LS[(SQLite/本地产物)]
        BU -->|Playwright/CDP| BR
...
| Browser Use Explore Agent | 在任务专属页面上执行 LLM 驱动的未知路径探索、页面理解和失败修复，并输出语义轨迹 | 作为永久生产脚本、持有任务凭据或绕过编排策略 |
...
flowchart TD
    ...
    M <-->|Loopback WS / Node IPC<br/>版本化 JSON Schema| P[Browser Use Explore Agent<br/>Python Sidecar]
...
- 管理 Browser Use Explore Agent 和 Playwright Runner 子进程。
- 维护任务状态机和控制权锁。
- 写入 SQLite、产物目录和审计日志。
...
#### Browser Use Explore Agent（Python Sidecar）

负责：

- 使用 Browser Use 的 Agent 与 Browser 抽象，根据结构化目标探索目标站点。
- 只通过任务专属 CDP 连接可见 Chromium；不得自行启动浏览器、连接用户日常 Chrome 或访问其他 Profile。
- 使用 OpenAI 兼容 LLM 客户端驱动工具循环，默认调用 DeepSeek；页面快照、可访问性树和任务字段在本地脱敏后才可发送给模型。
- 通过适配层输出动作提议、动作结果、页面状态、候选 Locator 与置信度，并将其转换为统一语义事件（Action Trace 输入）。
- 在不确定、失败、域名变化或敏感操作前请求编排器决策；Sidecar 不能绕过主进程直接执行高风险动作。

Browser Use Explore Agent 不负责：

- 直接上传数据到服务端。
- 读取客户端 Token、Cookie 文件或任意本地文件。
- 绕过本地编排器执行高风险操作。
- 作为已发布能力的生产执行器。
- 持久化 LLM API Key、任务数据库或浏览器 Profile；密钥仅由主进程从操作系统凭据库注入到启动环境。
...
### 9.3 LLM Provider 协议与默认模型

Browser Use Explore Agent 与云端 Chat/Planner 都通过平台的 OpenAI 兼容适配层调用模型；业务层不得依赖某一家 SDK 的私有消息格式。MVP 默认 Provider 为 DeepSeek，默认模型为配置项 `deepseek-v4-flash`，复杂页面诊断可经策略批准升级到 `deepseek-v4-pro`。模型名、Base URL、超时、重试与预算均由服务端策略下发，客户端只接受已签名的 Provider 配置。
...
- Browser Use 使用 `ChatOpenAI` 的自定义 OpenAI 兼容地址配置接入 DeepSeek；不得把 Provider 专有调用散落在 Agent 业务逻辑中。
- API Key 不写入任务、能力包、日志、崩溃报告或服务端数据库。个人/设备密钥保存在 Windows 凭据管理器；企业托管密钥只经服务端短期令牌或受控代理使用。
- Sidecar 发给模型的内容仅限任务目标、已脱敏页面摘要、可访问性树片段和当前语义上下文；密码、Cookie、验证码、完整页面源码、下载文件和无关个人信息不得发送。
- 模型调用必须记录 Provider、模型、请求版本、Token、耗时、重试次数与脱敏状态，不记录原始提示词和原始模型响应。
- Provider 不可用、模型超时或结构化输出不合格时，按预算重试；仍失败则进入 `WAITING_HUMAN`，不得无提示地切换到未批准模型。
- DeepSeek 的模型名和协议能力可能变化，发布版本在环境配置中锁定实际模型 ID，并在阶段 0 验证工具调用、JSON 输出和长页面摘要兼容性。
...
## 10. Browser Use Explore Agent 探索设计

Explore Agent 是系统中的逻辑角色，其实现为运行在本地的 Browser Use Python Sidecar。它结合 LLM 进行未知页面探索，但不替代确定性的 Playwright 能力执行。Sidecar 通过受限 IPC 接收任务快照，使用 Browser Use 连接任务专属可见 Chromium，并通过适配层把 Browser Use 的历史、动作与结果规范化为平台事件和 Action Trace。

MVP 使用 Browser Use 开源本地运行时，不使用 Browser Use Cloud、云端 Browser Profile、Cookie 同步或 Browser Use 托管的实时预览。用户浏览器、登录 Session 与截图流始终由本地客户端管理。

### 10.1 探索输入

Browser Use Explore Agent 接收：
...
Browser Use Sidecar 通过任务专属 CDP 连接任务浏览器，并在每一步经适配层发出结构化钩子事件：动作提议、动作结果、URL、提取内容、当前 Target 和模型调用摘要。系统利用这些钩子建立 Action Trace，而不是解析自然语言日志。若 Browser Use 所需操作不能映射到允许的语义动作，编排器必须拒绝、要求人工确认或将其标记为不可发布的探索证据。
...
超过预算后进入 `WAITING_HUMAN`，停止 Sidecar 的模型调用，不继续消耗模型资源。

## 11. Action Trace 与 Playwright 脚本生成

### 11.1 为什么需要 Action Trace

Browser Use Explore Agent 的执行历史适合观察，但不能直接等价为稳定脚本。系统需要建立中间表示，使探索过程和 Playwright 生产脚本解耦。
...
- Browser Use Explore Agent 模型动作。
- Browser Use Explore Agent 工具调用。
- Playwright/CDP 页面、网络和 Target 事件。
- 用户人工操作中的关键语义事件。
- 用户对字段映射的修正。
- 数据提取和校验结果。
...
- `score < 0.65`：使用 Browser Use Explore Agent 探索。
...
| 大文件上传 | 对象存储预签名 URL |
| 本地 TypeScript 进程通信 | Loopback WebSocket JSON 或 Node IPC |
...
- Browser Use Explore Agent 的 Token、步骤、时延和失败分类。
...
- 任务创建数和完成率。
- Browser Use Explore Agent 探索成功率。
- 能力匹配率和复用成功率。
- 回退探索率。
- 能力平均寿命和降级率。
- 人工介入率。
...
| Selector 失效 | 尝试 Fallback，失败后由 Browser Use Explore Agent 修复 |
| 登录失效 | 人工介入 |
| 验证码/MFA | 人工介入 |
| 页面指纹不符 | 停止能力执行，转探索或用户确认 |
| 数据校验失败 | 保留原始证据，转探索修正 |
| Browser Use Explore Agent 崩溃 | 重启一次并从安全检查点恢复 |
...
| 约束 | 决定 |
|---|---|
| 实现语言 | 核心业务 TypeScript；Browser Use Sidecar 为受控 Python 运行时 |
| B/S UI 框架 | Vue 3，不使用 React |
| 探索运行时 | Browser Use Python Sidecar + OpenAI 兼容 LLM 客户端，默认 DeepSeek |
| 共享方式 | monorepo 共享 packages（契约、Schema、SDK、策略引擎） |
| 允许的非 TS | 基础设施与浏览器内核（PostgreSQL、S3、Chromium），以及仅用于本地 Browser Use Sidecar 的锁定 Python 环境 |

### 20.1 B/S 前端

| 领域 | 选择 | 原因 |
|---|---|---|
| Web 框架 | Vue 3 + TypeScript + Vite | 三栏布局、Chat 流与复杂状态足够；团队统一 TS 且避开 React 生态分叉 |
| UI 状态 | Pinia | 管理布局、筛选和短生命周期视图状态 |
| 服务端状态 | TanStack Query（Vue） | 缓存、重试和失效管理 |
| 实时通信 | WebSocket | Chat 流、任务事件和截图帧 |
| Schema | Zod + JSON Schema | 与客户端、服务端共享边界校验 |
...
| 探索 Agent | Browser Use Python Sidecar | 未知站点探索；通过版本化 IPC 与 TypeScript 编排器协作，连接同一任务 Chromium |
| Sidecar 环境 | Python + Browser Use 锁定版本 | 通过 `uv.lock` 或等效锁文件确保可复现安装；由桌面安装包随应用分发 |
| 本地通信 | Node IPC / stdio JSON-RPC / Loopback WebSocket JSON | Node Runner 可用 Node IPC；跨语言 Sidecar 使用版本化 JSON Schema，不传递任意代码 |
| 日志 | Pino | 结构化日志 |
...
| LLM 接入 | OpenAI 兼容 Provider Adapter，默认 DeepSeek | Chat 规划与 Browser Use 统一协议；Provider、模型、预算和回退策略集中配置 |
| 验证 Worker | Node.js / TypeScript 独立进程 | 与 Playwright TypeScript 能力一致 |

### 20.4 暂不引入

- React 及 React 生态（Next.js、Redux、Zustand 等）。
- Browser Use 之外的 Python 业务运行时、Python 能力包依赖和服务端 Python Worker。
- Kubernetes。
- Kafka。
- Elasticsearch。
- 向量数据库。
...
│   │   ├── src/preload/
│   │   └── src/agent/
│   ├── browser-use-sidecar/      # Python Browser Use 探索进程入口与受限 IPC 适配器
│   ├── server/
│   │   └── src/
│   └── validator-worker/
├── packages/
...
│   ├── task-schema/
│   ├── capability-sdk/
│   ├── explore-core/             # 探索策略、Sidecar 事件适配与语义轨迹规范化
│   ├── llm-protocol/             # OpenAI 兼容请求、Provider 配置与脱敏规则
│   ├── playwright-runner/
│   └── policy-engine/
├── docs/
│   ├── adr/
...
| 契约测试 | OpenAPI、WebSocket 消息、截图帧、能力包 Schema、Sidecar JSON Schema 与 OpenAI 兼容 Provider 适配 |
...
3. Browser Use Explore Agent 找到模块并采集，B/S 工作区同步展示只读截图流。
4. 中途触发登录，客户端将本地浏览器切到前台，用户接管后继续。
5. 人工接管期间自动化无并发输入，恢复前重新验证页面状态。
6. 采集数据通过 Schema 校验。
7. 生成 Playwright 能力。
8. 在独立回放上下文连续回放两次成功（登录站允许同 Profile 已登录态，新 Page/新检查点）。
9. 能力包上传、扫描和发布。
10. 第二个同类任务自动匹配能力。
...
- 全仓 TypeScript 静态检查通过。
- 单元和集成测试通过。
- API 契约测试通过。
- 能力包 Schema 测试通过。
- B/S 页面不能向本地浏览器注入输入事件。
- Companion Agent、浏览器 Profile 和 CDP 隔离测试通过。
- 不存在 React 依赖；Python 依赖仅允许锁定在 `browser-use-sidecar`，且不得进入服务端、能力包或 Playwright Runner。
- Browser Use Sidecar 的锁文件、启动完整性校验和版本兼容矩阵通过；CI 使用本地 OpenAI 兼容 Mock 验证 Provider 契约，不依赖真实模型服务。
- 核心业务模块覆盖率不低于 80%。
- 测试站点 E2E 全部通过。
- 无高危依赖和脚本扫描问题。
...
- Web 端“打开本地浏览器”入口。
- Chat 结构化任务定义。
- Browser Use Explore Agent 探索，并使用默认 DeepSeek 模型完成 LLM 工具循环。
- 登录/MFA 人工接管。
- 右侧步骤和关键日志。
- 表格、分页、列表加详情采集。
...
- 复杂反爬规避。
- React 或其他前端框架迁移。
- Browser Use 之外的 Python 业务运行时，以及未通过 OpenAI 兼容适配层的模型直连。
...
- 验证 Browser Use Python Sidecar、DeepSeek OpenAI 兼容调用与本地可见 Chromium 的连接。
- 验证 Browser Context、Page、Profile 和 CDP 隔离。
- 验证截图采集、脱敏、WebSocket 转发和 Vue 三栏工作区展示。
- 验证 B/S 触发本地浏览器切前台以及控制权切换。
- 验证 monorepo 共享类型在 Web / desktop / server 间可编译复用。

验收：

- Browser Use Explore Agent 能在本地可见 Chromium 完成导航、点击和提取。
- Browser Use 能通过 OpenAI 兼容协议调用配置的 DeepSeek 模型，并对工具调用与结构化事件完成 Schema 校验。
- B/S 工作区能以 2 至 5 FPS 展示只读画面，且不能注入输入事件。
- 用户能在本地浏览器接管并恢复。
- Playwright Runner 能在同一页面执行基础 Locator 和提取。
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| Browser Use 与本地 Chromium/CDP 兼容性不足 | 首次探索受阻 | 阶段 0 Spike，锁定 Browser Use、Python 与 Playwright 版本；不通过则停止阶段 1 并评估适配器替代方案 |
| Playwright CDP API 保真度不足 | 部分脚本无法本地复用 | 限制 API 子集，服务端用原生 Playwright 验证 |
| Browser Use + DeepSeek 的工具调用或页面理解效果不足 | 探索成功率偏低 | 受控测试站验收、页面摘要脱敏、工具白名单和模型预算迭代；必要时在不改变 OpenAI 兼容协议的前提下切换已批准模型 |
| Browser Use Python 环境无法随 Electron 稳定分发或升级 | 客户端无法启动探索 | 将 Python、Browser Use 与锁文件纳入桌面安装包；启动时执行完整性和版本兼容检查，失败时禁止任务进入探索 |
| Action Trace 编译质量不足 | 能力无法发布 | 限制 MVP 页面模式，允许“采集成功但不发布能力” |
...
| Electron + Playwright Chromium 体积大 | 安装包和升级成本 | 固定版本、按需下载浏览器运行时、健康检查和原子升级 |
...
### ADR-002：Browser Use Explore Agent 只负责探索和修复

状态：接受。

决定：

- Browser Use Explore Agent 只运行在本地 Sidecar 中，不作为稳定任务的唯一执行器。
- 探索与生产执行分离：探索用 Browser Use + LLM，复用用 Playwright 能力包。
- Browser Use 通过 OpenAI 兼容协议调用默认 DeepSeek，Provider 和模型可在不改 Agent 业务逻辑的前提下替换。
...
### ADR-008：TypeScript 核心系统 + Browser Use Python Sidecar，B/S 不使用 React

状态：接受。

决定：

- Web、Companion Agent、Playwright Runner、服务端、验证 Worker 与共享契约统一 TypeScript。
- B/S 采用 Vue 3 + TypeScript + Vite，不引入 React。
- 探索能力使用本地 Browser Use Python Sidecar；Python 不得进入服务端、能力包和稳定执行 Runner。
- Browser Use 的 LLM 调用使用 OpenAI 兼容协议，MVP 默认 Provider 为 DeepSeek。

原因：

- 核心业务仍可让任务 Schema、事件、能力包类型单源共享；Sidecar 通过生成的 JSON Schema 避免跨语言 DTO 漂移。
- Browser Use 已提供成熟的 LLM 浏览器探索循环；限制 Python 边界可避免其渗入稳定执行与服务端运维。
- Browser Use 探索与 Playwright 执行经 Action Trace 解耦，控制权和 Locator 策略由 TypeScript 编排器统一约束。
- 明确排除 React，避免前端生态分叉和重复状态管理方案。
...
3. Browser Use Explore Agent 能通过 OpenAI 兼容协议调用已批准的 DeepSeek 模型，在本地页面探索并实时输出步骤。
...
10. 生成脚本能在独立回放上下文连续回放两次。
11. 能力包通过安全扫描、上传并发布。
12. 第二次同类任务能够匹配和复用能力。
13. 页面变化导致能力失效时能够安全回退到 Browser Use Explore Agent。
14. 能力、任务和人工介入均有可审计记录。
15. 核心业务为 TypeScript monorepo，B/S 不依赖 React；Python 仅存在于锁定版本的本地 Browser Use Sidecar，且模型调用只走 OpenAI 兼容适配层。
...
- [DeepSeek API：OpenAI 兼容接入](https://api-docs.deepseek.com/guides/agent_integrations/openclaw)
